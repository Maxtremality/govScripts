...
async def get_data(url, token, params):
    headers = {'Authorization': token}
    query = requests.get(url, params=params, headers=headers)
    data = json.loads(query.text)['result']
...
async def waybill_get(token: str, params):
    headers = {'Authorization': token}
    req_data = requests.get(settings.WAYBILL_URL, params=params, headers=headers)
