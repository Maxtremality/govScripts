...
def data_processing(username, waybills, cars, average_consumption_data, estp, exceptions):
    result = []
    for waybill in waybills['result']:
        if any(exc["okrug_name"] == waybill['okrug_name'] and exc["company_name"] == waybill['company_name'] and exc["number"] == waybill['number'] for exc in exceptions) is False:
...
async def get_estp_data(session, waybill):
    if waybill['company_name'] == 'АвД ВАО':
        ...
        headers_estp = {
            'Cookie': 'PHPSESSID=46anc3km02k157oes0ulbht3s6; _identity=25440d631f24729c37aba24f528168492f6f09ae913e7e6ad7135124d09fb449a%3A2%3A%7Bi%3A0%3Bs%3A9%3A%22_identity%22%3Bi%3A1%3Bs%3A48%3A%22%5B615%2C%22lWLvLrhFBFmZBQbY87v6J9LTONB5rR4J%22%2C2592000%5D%22%3B%7D; _csrf=bc133f136d70530c3855e1b97e896b86f963f4be7c4f195617d77b8e74db7486a%3A2%3A%7Bi%3A0%3Bs%3A5%3A%22_csrf%22%3Bi%3A1%3Bs%3A32%3A%229i6CCAqOeKMqaWbDvnht2acg84LxxEKO%22%3B%7D',
            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/115.0.0.0 Safari/537.36'
        }
...
async def main():
    ...
    user_keys = ['id', 'username', 'password', 'okrug_id', 'company_id', 'okrug_name']
    users = [dict(zip(user_keys, row)) for row in users_tuple]
    exceptions_tuple = await data_get('*', 'api_etswaybillsexceptions', '', settings.DB_API, 'public', settings.DB_ACCESS)
    exceptions_keys = ['id', 'okrug_name', 'company_name', 'number']
    exceptions = [dict(zip(exceptions_keys, row)) for row in exceptions_tuple]
    ...
    with ThreadPoolExecutor() as executor:
        ...
        tasks_dp = [loop.run_in_executor(executor, partial(data_processing, username, waybills[username], cars[username], average_consumption_dict, estp, exceptions)) for username in waybills]
